...
for each  currentDocument in documents
{
	...
	info currentDocumentId;
	info mp;
	info zohoSignDocumentSearchRecord;
	values = zoho.crm.getOrgVariable("qualitycertification__eSignModuleName");
	info values;
	if(zohoSignDocumentSearchRecord.get("response") != null && zohoSignDocumentSearchRecord.get("response").length() > 3)
	{
		...
		for each  moduleNames in values
		{
			info moduleNames.get('apiLookupName');
			apiName = moduleNames.get('apiLookupName');
			moduleData = zohoSignDocumentSearchRecord.get(apiName);
			info moduleData;
			if(!moduleData.isNull() && !moduleData.isEmpty())
			{
				module_Id = moduleData.get('id');
				moduleApiName = moduleNames.get('moduleApiName');
			}
		}
		...
		if(operation_type.equalsIgnoreCase("RequestCompleted") || operation_type.equalsIgnoreCase("RequestExpired") || operation_type.equalsIgnoreCase("RequestRejected"))
		{
			for each  currentDocuments in documents
			{
				...
				if(isFile(resp))
				{
					...
					if(module_Id != "")
					{
						attachDoc = zoho.crm.attachFile(moduleApiName,module_Id,resp);
						info "\n\n Attached " + moduleApiName + "----" + attachDoc;
					}
					if(contact_Id != "" && module_Id == "")
					{
